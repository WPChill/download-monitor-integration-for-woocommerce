 ...
 *
 * This file contains the DLMWC_Integration class which integrates Download Monitor with WooCommerce.
 *
...
class DLMWC_Integration {
	 ...
	 *
	 * @return object The DLMWC_Integration object.
	 * @since 1.0.0
	 */
	public static function get_instance() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof DLMWC_Integration ) ) {
			self::$instance = new DLMWC_Integration();
		}
	...
	public function save_download_monitor_field( $post_id ) {
		...
		$download_monitor_ids = ! empty( $_POST[ DLMWC_Constants::META_WC_PROD_KEY ] ) ? array_map( 'absint', $_POST[ DLMWC_Constants::META_WC_PROD_KEY ] ) : array();// phpcs:ignore WordPress.Security.NonceVerification.Missing
		if ( ! empty( $download_monitor_ids ) ) {
			update_post_meta( $post_id, DLMWC_Constants::META_WC_PROD_KEY, $download_monitor_ids );
			// Lock each download to the product.
			foreach ( $download_monitor_ids as $id ) {
				$currently_locked = get_post_meta( absint( $id ), DLMWC_Constants::META_WC_LOCKED_KEY, true );
				if ( ! empty( $currently_locked ) && is_array( $currently_locked ) ) {
					if ( ! in_array( $post_id, $currently_locked, true ) ) {
						$currently_locked[] = $post_id;
					}
					update_post_meta( absint( $id ), DLMWC_Constants::META_WC_LOCKED_KEY, $currently_locked );
				} else {
					update_post_meta( absint( $id ), DLMWC_Constants::META_WC_LOCKED_KEY, array( $post_id ) );
				}
			}
		}
	}
	...
	public function show_download_monitor_content() {
		...
		foreach ( $orders as $order ) {
			...
			foreach ( $items as $item ) {
				...
				$downloads  = get_post_meta( $product_id, DLMWC_Constants::META_WC_PROD_KEY, true );
				if ( ! empty( $downloads ) ) {
	...
	public function enqueue_admin_scripts() {
		...
		wp_enqueue_script( 'dlm-wc-integration', DLMWC_URL . '/assets/js/admin.js', array( 'select2' ), DLMWC_VERSION, true );
		wp_enqueue_style( 'dlm-wc-integration', DLMWC_URL . '/assets/css/admin.css', array(), DLMWC_VERSION );
	}
}
